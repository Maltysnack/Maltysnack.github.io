# games HANDOFF

Index for `/games/`. Each game owns its own HANDOFF when non-trivial. Root `HANDOFF.md` covers site-wide.

## Subprojects

- [`magic/HANDOFF.md`](magic/HANDOFF.md): Magic meta-explorer (Archetypes tab live). Synergy-score now lives inside here.
- [`duoclue/HANDOFF.md`](duoclue/HANDOFF.md): Duoclue
- [`dnd/HANDOFF.md`](dnd/HANDOFF.md): D&D character sheet subsystem

## Current state (as of 18-05-2026)

- Folder reorg 18-05-2026: `games/magic.{html,js,css}` + `games/data/` + `games/synergy-score.html` collapsed into `games/magic/`. `projects/duoclue/` moved to `games/duoclue/`. `/dnd/` moved to `/games/dnd/`. Old paths deleted outright (no redirects: site has no inbound traffic).

## Gotchas

- macOS case-insensitive FS: keep folder names lowercase and consistent.
