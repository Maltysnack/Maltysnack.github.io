...
(function () {
  const DATA_DIR = "/games/magic/data";
  ...
  // "archetypes" = NMF-derived clusters + named archetypes (local preview only).
  const VALID_TABS = ["cards", "pros", "stories", "archetypes"];
  ...
  function render() {
    ...
    if (currentTab === "archetypes") wireArchSearch();
    wireTabNav();
    wireCardClicks();
  }
  ...
  function renderTabNav() {
    ...
    return `
      <nav class="tab-nav" aria-label="Sections">
        ...
        ${tab("archetypes", "Archetypes", "packages, archetype mixes")}
      </nav>
    `;
  }
  ...
  function renderTabContent() {
    ...
    if (currentTab === "archetypes") return renderArchetypesTab();
  ...
  // ── Archetypes tab (local preview) ───────────────────────────────────
  // Three-tier model:
  //   archetype  (human-named, from pro deck labels)
  //   cluster    (NMF-discovered card package; appears across archetypes)
  //   card       (handled in Cards tab)
  // The tab lets you browse archetypes top-down or clusters top-down, and
  // each entity cross-links to the others.
  // Archetype matches: by NAME ONLY. Member-card matching produced too much
  // noise; typing "llanowar" surfaced 16 archetypes because Llanowar Elves
  // appears in many green clusters. To find archetypes-by-card, the user
  // looks at the cluster section below (which still matches by card name).
  function archetypeMatchesQuery(a, q) {
    if (!q) return true;
    return a.name.toLowerCase().includes(q.toLowerCase());
  }

  // Cluster matches: by member card name, OR by being used by an archetype
  // whose name matches the query. The second condition lets "landfall" find
  // the Mono-Green Landfall clusters even though no card has "Landfall" in
  // its name.
  function clusterMatchesQuery(c, q) {
    ...
    for (const a of clusters.archetypes) {
      if (!a.name.toLowerCase().includes(ql)) continue;
      if (a.cluster_mix.some((x) => x.cluster === c.id)) return true;
    }
    return false;
  }

  function renderArchetypesTab() {
    if (!clusters) {
      return `<div class="loading">no cluster data available. run games/magic/data/cluster.py to generate clusters.json.</div>`;
    }
    const q = (searchQuery || "").trim();
    return `
      <section class="sec arch-sec">
        <header class="sec-header">
          <h2 class="sec-title">Archetypes &amp; clusters</h2>
          <span class="sec-sub">${clusters.n_clusters} clusters across ${clusters.archetypes.length} named archetypes · classification ${(clusters.archetype_classification_accuracy * 100).toFixed(0)}%</span>
        </header>
        <div class="arch-intro">
          Archetypes are human-named bundles from pro deck labels. Clusters are mathematically-discovered card packages that travel together. A deck is a mixture of clusters, with one or two dominant ones.
        </div>
        ${renderArchSearch(q)}
        ${renderArchetypeList(q)}
        ${renderClusterList(q)}
      </section>
    `;
  }

  // Tab-local search. Filters the archetype list and cluster list by name
  // or member-card name. Different semantics from the Cards/Pros search
  // (which adds to selection); here it narrows what is visible.
  function renderArchSearch(q) {
    return `
      <div class="search-shell arch-search-shell">
        <input class="search-input arch-search-input" type="text" placeholder="filter archetypes and clusters… try a card name or archetype keyword" autocomplete="off" spellcheck="false" value="${escapeAttr(q)}">
      </div>
    `;
  }

  function clusterById(id) {
    return clusters && clusters.clusters.find((c) => c.id === id);
  }

  function clusterTopCardName(id) {
    const c = clusterById(id);
    return (c && c.members[0] && c.members[0].name) || id;
  }

  // Compact label for a cluster: top card name truncated.
  function clusterChipLabel(id) {
    const c = clusterById(id);
    if (!c || !c.members.length) return id;
    const top = c.members[0].name;
  ...
  // Helper: top card image for a cluster, plus its name for tooltip.
  function clusterKeyImg(id) {
    const c = clusterById(id);
    const top = c && c.members[0];
    return top ? { name: top.name, src: img(top.name) } : null;
  }

  function renderArchetypeCard(a) {
    const isOpen = selectedArchetype === a.name;
    // Closed: show only the top 2 clusters so very-mixed archetypes (Temur
    // Harmonizer has 6) do not balloon the tile height. Open: show all.
    const mixToShow = isOpen ? a.cluster_mix : a.cluster_mix.slice(0, 2);
    const moreCount = a.cluster_mix.length - mixToShow.length;

    const mixBars = mixToShow.map((m) => {
      const pct = Math.round(m.weight * 100);
      const key = clusterKeyImg(m.cluster);
      const imgHtml = key && key.src
        ? `<img class="arch-mix-img" src="${key.src}" alt="" data-preview="${escapeAttr(key.name)}" title="${escapeAttr(key.name)}">`
        : `<span class="arch-mix-noimg">${escapeHtml(m.cluster)}</span>`;
      return `<div class="arch-mix-row">
        ${imgHtml}
        <div class="arch-mix-bar-track"><span class="arch-mix-bar" style="width:${Math.min(pct, 100)}%"></span></div>
        <span class="arch-mix-pct">${pct}%</span>
      </div>`;
    }).join("");

    const moreHint = (!isOpen && moreCount > 0)
      ? `<div class="arch-mix-more">+${moreCount} more (click to expand)</div>`
      : "";

    const detail = isOpen ? `
      <div class="arch-detail">
        <div class="arch-detail-label">cluster composition (click a cluster to jump to it)</div>
        <div class="arch-detail-chips">
          ${a.cluster_mix.map((m) => {
            const key = clusterKeyImg(m.cluster);
            const pct = Math.round(m.weight * 100);
            const imgHtml = key && key.src
              ? `<img class="arch-chip-img" src="${key.src}" alt="" data-preview="${escapeAttr(key.name)}" title="${escapeAttr(key.name)}">`
              : `<span class="arch-chip-noimg">${escapeHtml(m.cluster)}</span>`;
            return `<button class="arch-cluster-card-chip" data-cluster-pick="${escapeAttr(m.cluster)}" title="${escapeAttr(key ? key.name : m.cluster)}">
              ${imgHtml}
              <span class="arch-chip-pct">${pct}%</span>
            </button>`;
          }).join("")}
        </div>
      </div>
    ...
    return `
      <article class="arch-card${isOpen ? " open" : ""}" data-arch-pick="${escapeAttr(a.name)}">
        <header class="arch-card-head">
          <h4 class="arch-card-title">${escapeHtml(a.name)}</h4>
          <span class="arch-card-count">${a.deck_count}</span>
        </header>
        <div class="arch-mix">${mixBars}</div>
        ${moreHint}
        ${detail}
      </article>
    `;
  ...
  function renderClusterList(q) {
    const list = clusters.clusters.filter((c) => clusterMatchesQuery(c, q));
    if (q && !list.length) {
      return `<div class="arch-block">
        <h3 class="arch-block-title">Clusters</h3>
        <div class="rec-empty">no cluster contains a card matching "${escapeHtml(q)}"</div>
      </div>`;
    }
    return `
      <div class="arch-block">
        <h3 class="arch-block-title">Clusters${q ? ` <span class="arch-filter-note">filtered: ${list.length}</span>` : ""}</h3>
        <div class="arch-intro arch-intro-sub">
          Clusters are numbered (c0&ndash;c${clusters.n_clusters - 1}). The biggest card in each cluster is the strongest member.
        </div>
        <div class="cluster-grid">
          ${list.map((c) => renderClusterCard(c)).join("")}
        </div>
      </div>
    `;
  }
  ...
  function renderClusterCard(c) {
    const isOpen = selectedCluster === c.id;
    const selSet = new Set(selection);

    // Closed: simple 2×2 preview of the top 4 members, uniform thumbnails.
    // Open: weight-scaled dense grid showing all members. The growth from
    // "what's in here roughly" → "the full picture with relative weights"
    // is the value of the click.
    let tilesHtml;
    if (!isOpen) {
      const preview = c.members.slice(0, 4);
      tilesHtml = `<div class="cluster-preview">${preview.map((m) => {
        const im = img(m.name);
        const inSel = selSet.has(m.name);
        return `<div class="cluster-preview-tile${inSel ? " in-sel" : ""}" data-name="${escapeAttr(m.name)}" data-preview="${escapeAttr(m.name)}" title="${escapeAttr(m.name)}">
          ${im ? `<img src="${im}" alt="" loading="lazy">` : `<div class="cluster-tile-noimg">${escapeHtml(m.name)}</div>`}
        </div>`;
      }).join("")}</div>`;
    } else {
      // Each tile spans n×n cells of a grid whose column-width and row-height
      // are in the card's 488/680 aspect ratio (so n×n always renders the
      // full card image at the correct shape, no cropping). Three sizes:
      //   anchor (top card):    6×6 cells
      //   medium (next 5 cards): 4×4 cells
      //   small (rest):          2×2 cells
      // These tessellate with dense grid flow: a single anchor leaves an
      // L-shape that mediums and smalls fill cleanly.
      tilesHtml = `<div class="cluster-tiles">${c.members.map((m, i) => {
        const im = img(m.name);
        const span = i === 0 ? 6 : i <= 5 ? 4 : 2;
        const inSel = selSet.has(m.name);
        return `<div class="cluster-tile${inSel ? " in-sel" : ""}" style="grid-column:span ${span};grid-row:span ${span}" data-name="${escapeAttr(m.name)}" data-preview="${escapeAttr(m.name)}" title="${escapeAttr(m.name)} · NMF weight ${m.weight}">
          ${im ? `<img src="${im}" alt="" loading="lazy">` : `<div class="cluster-tile-noimg">${escapeHtml(m.name)}</div>`}
        </div>`;
      }).join("")}</div>`;
    }
  ...
  function renderBreakdownPopover(name) {
    ...
    if (!entry.breakdown) {
      return `<div class="bd-popover" data-bd="1">
        ...
        <a class="bd-link" href="/games/magic/synergy-score.html" data-bd-stop>read about synergy score →</a>
      </div>`;
    }
  ...
  // Archetypes-tab search has filter-only semantics: typing narrows the
  // visible archetypes/clusters; it does NOT add cards to selection. Enter
  // does nothing (the user is reading, not picking).
  function wireArchSearch() {
    const input = $(".arch-search-input");
    if (!input) return;
    if (searchQuery) {
      input.focus();
      const len = input.value.length;
  ...
  function wireCardClicks() {
    ...
    // Archetype card click: toggle expansion. If we just opened (or jumped
    // to) an archetype, scroll it into view; the "used by archetypes" chips
    // live in the cluster section below the archetype list, so clicking
    // them silently expanded an archetype above with no visible feedback.
    $$("[data-arch-pick]").forEach((el) => {
      el.addEventListener("click", (e) => {
        e.stopPropagation();
        const name = el.dataset.archPick;
        if (el.dataset.clusterPick) return;
        selectedArchetype = selectedArchetype === name ? null : name;
        render();
        if (selectedArchetype) {
          const target = $(`.arch-card[data-arch-pick="${name}"]`);
  ...
  async function loadInitial() {
    // stories.json and clusters.json are optional; if 404 we just hide them
    const results = await Promise.all([
      loadJson("meta"),
      loadJson("explore"),
      loadJson("scryfall"),
      loadJson("stories").catch(() => null),
      loadJson("clusters").catch(() => null),
    ]);
    [meta, explore, scryfall, stories, clusters] = results;
  }
