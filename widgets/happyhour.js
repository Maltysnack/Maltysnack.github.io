...
// Shows a city currently in happy hour (4pm–6pm local) with a photo,
// time, temp, and what locals would order. Refreshes ~every 15 min.
//
// To install: long-press the home screen → "+" → Scriptable → choose the
// medium widget size → tap the placeholder widget → Script: "happyhour".
...
async function fetchPayload() {
  ...
  for (const city of eligible) {
    try {
      ...
      return { city, img, temp, code, time, drinks };
    } catch (e) {
      continue;
    }
  }
...
function buildWidget(data) {
  ...
  if (!data) {
    // Empty state, point at coffeetime
  ...
  const { city, img, temp, code, time, drinks } = data;
  ...
  const order = drinks[0] ? `${time}  ·  ${drinks[0]}` : time;
  const meta = w.addText(order);
